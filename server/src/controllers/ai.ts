...
export async function handleBotQuery(req: AuthRequest, res: Response) {
  ...
  console.log(`[AI Query] User ${userId} in channel ${channelId}: "${content}"`);
  ...
  try {
    ...
    console.log(`[AI Query] Found ${channelResults.length} channel results and ${workspaceResults.length} workspace results`);

    // Combine and deduplicate results
    const allResults = [...channelResults, ...workspaceResults]
