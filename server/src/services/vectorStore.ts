...
export class VectorStoreService {
  ...
  private async getStoreNamespace({ type, workspaceId, channelId }: VectorStoreConfig): Promise<string> {
    ...
    try {
      ...
      const result = await client.query(
        `INSERT INTO vector_stores (type, workspace_id, channel_id, name)
         VALUES ($1, $2, $3, $4)
         RETURNING id`,
        [config.type, config.workspaceId, config.channelId, await this.getStoreNamespace(config)]
      );
  ...
  async addDocuments(config: VectorStoreConfig, documents: string[]): Promise<void> {
    const namespace = await this.getStoreNamespace(config);
    console.log(`[VectorStore] Adding ${documents.length} documents to namespace: ${namespace}`);
    
    try {
      // Verify index connection
      const indexStats = await this.index.describeIndexStats();
      console.log(`[VectorStore] Connected to index. Current stats:`, indexStats);
      ...
      for (let i = 0; i < documents.length; i += batchSize) {
        ...
        console.log(`[VectorStore] Processing batch ${i/batchSize + 1}, size: ${batch.length}`);
        await vectorStore.addDocuments(batch.map(content => ({
          pageContent: content,
          metadata: { timestamp: new Date().toISOString() }
        })));
      }

      console.log(`[VectorStore] Successfully added all documents to namespace: ${namespace}`);
    } catch (error) {
      console.error(`[VectorStore] Failed to add documents to namespace: ${namespace}`, error);
      throw error;
    }
  }
  ...
  async namespaceExists(config: VectorStoreConfig): Promise<boolean> {
    try {
      console.log(`[VectorStore] Checking if namespace exists jl: ${config.type} ${config.workspaceId || config.channelId}`);
      const namespace = await this.getStoreNamespace(config);
