...
export class WebSocketHandler {
  ...
  private async broadcastPresenceUpdate(userId: string, isOnline: boolean) {
    try {
      const user = await pool.query('SELECT username, last_seen FROM users WHERE id = $1', [userId]);
  ...
  private async handleChatMessage(ws: WebSocketClient, data: ChatMessage) {
    try {
      const user = await pool.query('SELECT username FROM users WHERE id = $1', [ws.userId]);
      ...
      if (data.isDM) {
        ...
        if (dmCheck.rows.length === 0) {
          ws.send(JSON.stringify({ type: 'error', error: 'Not authorized for this DM' }));
          return;
        }

        // Save DM message
        await pool.query(
          'INSERT INTO messages (content, user_id, dm_id, created_at) VALUES ($1, $2, $3, NOW())',
        ...
        if (channelCheck.rows.length === 0) {
          ws.send(JSON.stringify({ type: 'error', error: 'Not authorized for this channel' }));
          return;
        }
  ...
  public async handleChannelJoin(ws: WebSocketClient, channelId: string) {
    try {
      ...
      ws.send(JSON.stringify({ type: 'error', error: 'Failed to join channel' }));
    }
  }
